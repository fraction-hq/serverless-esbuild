...
function getProdModules(
  ...
  R.forEach(
    (
      ...
      const modulePackage: Partial<PackageJSON> =
        modulePackagePath
      ...
      try {
        ...
        const peerDependencies =
          modulePackage.peerDependencies as Record<
            string,
            string
          >;
          ...
          Object.keys(
            R.pickBy(
              (
                ...
                val.optional,
              modulePackage.peerDependenciesMeta ||
                {}
            )
          );
        ...
        assert(
          this
            .buildOptions,
          "buildOptions not defined"
        );

        const peerDependenciesWithoutOptionals =
          R.omit(
            [
...
export function nodeExternalsPluginUtilsPath():
  | string
  | undefined {
  try {
    const resolvedPackage =
      require.resolve(
        "esbuild-node-externals/dist/utils",
        {
          paths:
            [
              process.cwd(),
...
interface ParsedExternals {
  ...
  names: string[];
  /** Map of package name to install args (e.g., "--os=linux --cpu=x64") */
  installArgs: Map<
    string,
    string
  >;
...
async function runPostinstallScripts(
  ...
  for (const [
    ...
    if (
      ...
      plugin.log.warning(
        `Skipping postinstall for ${packageName}: package directory not found at ${packageDir}`
      );
      continue;
    }
    ...
    plugin.log.verbose(
      `Running postinstall for ${packageName}: ${expandedScript}`
    );
    ...
    try {
      ...
      plugin.log.debug(
        `Postinstall for ${packageName} completed in ${
          Date.now() -
          startTime
        }ms`
      );
      ...
      plugin.log.error(
        `Postinstall script failed for ${packageName}: ${
          error instanceof
          Error
            ? error.message
            : error
 ...
 */
async function installExternalNestedDependencies(
  plugin: EsbuildServerlessPlugin,
  compositeModulePath: string,
  externals: string[],
  packager: Awaited<
...
async function installPackages(
  ...
  if (
    ...
    for (const dep of installDeps) {
      plugin.log.verbose(
        `Installing ${dep.package} at ${compositeModulePath} with NPM`
      );
  ...
  // Install nested dependencies for each external package
  await installExternalNestedDependencies(
    plugin,
    compositeModulePath,
    externals,
    packager
...
export async function packExternalModules(
  ...
  const rawExternals =
    this
      .buildOptions
      .external;
  let parsedExternals: ParsedExternals =
    {
      names:
        [],
      installArgs:
  ...
  for (const [
    pkg,
    args,
  ] of parsedExternals.installArgs) {
    if (
      externals.includes(
        pkg
      )
  ...
  for (const [
    pkg,
    script,
  ] of parsedExternals.postinstallScripts) {
    if (
      externals.includes(
        pkg
      )
  ...
  assert(
    packageJsonPath,
    "packageJsonPath is not defined"
  );
  ...
  const sectionNames =
    packager.copyPackageSectionNames;
  ...
  if (
    ...
    this.log.debug(
      `Using package.json sections ${R.join(
        ", ",
        R.keys(
          packageSections
        )
  ...
  this.log.debug(
    `Fetch dependency graph from ${packageJson}`
  );
  ...
  const compositeModules: JSONObject =
    R.uniq(
      getProdModules.call(
        this,
        externalModules,
  ...
  const compositeModulePath =
    this
      .buildDirPath;

  assertIsString(
    compositeModulePath,
    "compositeModulePath is not a string"
  );

  const compositePackageJson =
    path.join(
      compositeModulePath,
      "package.json"
    );
  ...
  this.log.verbose(
    `Packing external modules: ${compositeModules.join(
      ", "
    )}`
  );
  const {
